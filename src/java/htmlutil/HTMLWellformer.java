...
import static htmlutil.SyntaxState.*;
...
//                        writer.append(content.toString().trim());
                ...
                case CLOSE_BRACKET:
                    if (isOpenTag) {
                        ...
                        writer.append(LT).append(openTagName).append(convert(attributes)).append((isEmptyTag ? "/" : "")).append(GT);
                        attributes.clear();
                        if (!isEmptyTag) {
                        ...
                        if (!stack.isEmpty() && stack.contains(closeTagName)) {
                            while (!stack.isEmpty() && !stack.peek().equals(closeTagName)) {
                                writer.append(LT).append(SLASH).append(stack.pop()).append(GT);
                            }
        ...
        if (CONTENT.equals(state)) {
            writer.append(content.toString().trim().replace("&", "&amp"));
//            writer.append(content.toString().trim());
        }
        while (!stack.isEmpty()) {
            writer.append(LT).append(SLASH).append(stack.pop()).append(GT);
    ...
    private String convert(Map<String, String> attributes) {
        ...
        for (Map.Entry<String, String> entry : attributes.entrySet()) {
            String value = entry.getValue().replace("&", "&amp;").replace("\"", "&quote;").replace("'", "&apos;").replace("<", "&lt;").replace(">", "&gt;");
